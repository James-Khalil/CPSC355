...
newPyramid:         stp x29, x30, [sp, -16]!                              // newPyramid subroutine initializes the values

                                                                        // x3 is the address for the pyramid structure in question
                                                                        // so we offset based on what pyramid value we want

                    str wzr, [x3, pyramid_center + coord_x]         // Coordinate x starts at 0
                    str wzr, [x3, pyramid_center + coord_y]         // Coordinate y starts at 0
                    str w0, [x3, pyramid_base + size_width]     // Set the width to the defined argument
                    str w1, [x3, pyramid_base + size_length]    // Set the length to the defined argument
                    str w2, [x3, pyramid_height]                // Set the height to the defined argument

                                                                        // Volume is calcualted using width, lenght, and height
                    ldr w20, [x3, pyramid_base + size_width]        // Load the value of width into w20
                    ldr w21, [x3, pyramid_base + size_length]       // Load the value of length into w21 
                    ldr w22, [x3, pyramid_height]                   // Load the value of height into w22
                    ...
                    str w20, [x3, pyramid_volume]                   // Store the result at the pyramid volume offset
...
relocate:           stp x29, x30, [sp, -16]!                              // Relocate subroutine changes the coordinates

                    ldr w19, [x3, pyramid_center + coord_x]         // Load coordiante x into w19
                    add w19, w19, w0                                // Add the first arguemnt to w19
                    str w19, [x3, pyramid_center + coord_x]         // Store w19 into coordinate x in stack

                    ldr w19, [x3, pyramid_center + coord_y]         // Load coordinate y into w19
                    add w19, w19, w1                                // Add the second argument to w19
                    str w19, [x3, pyramid_center + coord_y]         // Store w19 into coordinate y in stack

                    ldp x29, x30, [sp], 16	                            // Loads the pair of registers and deallocates 16 bytes of memory from the stack
	                ret			                                        // Returns to calling code (in OS)

expand:             stp x29, x30, [sp, -16]!                              // subroutine for increasing the base, width, and height by a factor
                    mov	x29, sp		                                    // Update FP to the current SP

                    ldr w19, [x3, pyramid_base + size_width]        // load the width into w19
                    mul w19, w19, w0                                // Multiply w19 by the argument
                    str w19, [x3, pyramid_base + size_width]        // Store w19 back into width
                    
                    ldr w20, [x3, pyramid_base + size_length]       // load the length into w20
                    mul w20, w20, w0                                // Multiply w20 by the factor
                    str w20, [x3, pyramid_base + size_length]       // Store w20 back into length

                    ldr w21, [x3, pyramid_height]                   // Load the height into w21
                    mul w21, w21, w0                                // Multiply w21 by the factor
                    str w21, [x3, pyramid_height]                   // Store w21 back into height
                    ...
                    str w19, [x3, pyramid_volume]                   // Store the result in volume

                    ldp x29, x30, [sp], 16	                            // Loads the pair of registers and deallocates 16 bytes of memory from the stack
	                ret			                                        // Returns to calling code (in OS)


printPyramid:       stp x29, x30, [sp, -16]!                              // subroutine for printing pyramid information

                    mov x19, x3                                     // Move the address argument into x19
                                                                        // x3 will be overwritten when we branch into printf

                    mov w1, w0                                          // Move w0 into w1 before it is overwritten on line 123
...
equalSize:          stp x29, x30, [sp, -16]!                              // subroutine for checking if the pyramids are of equal size

                    w0 = FALSE                                          // Intialize w0 to FALSE. If not changed, w0 will be returned as FALSE
                    ldr w19, [x3, pyramid_base + size_width]        // Load the width of pyramid 1 into w19
                    ldr w20, [x4, pyramid_base + size_width]        // Load the width of pyramid 2 into w20
                    cmp w19, w20                                        // Compare w19 and w20
                    b.ne exit_equalSize                                 // If not equal skip to end of subroutine

                    ldr w19, [x3, pyramid_base + size_length]       // Load the length of pyramid 1 into w19
                    ldr w20, [x4, pyramid_base + size_length]       // Load the length of pyramid 2 into w20
                    cmp w19, w20                                        // Compare w19 and w20
                    b.ne exit_equalSize                                 // If not equal skip to the end of subroutine

                    ldr w19, [x3, pyramid_height]                   // Load the height of pyramid 1 into w19
                    ldr w20, [x4, pyramid_height]                   // Load the height of pyramid 2 into w20
                    cmp w19, w20                                        // Compare w19 and w20
                    b.ne exit_equalSize                                 // If not equal skip to the end of subroutine
...
main:               stp x29, x30, [sp, alloc]!                            // Main function, will call all other subroutines from here
                    ...
                    mov w0, 10                                      // w0 = 10 (used for width)
                    mov w1, 10                                      // w1 = 10 (used for length)
                    mov w2, 9                                       // w2 = 9 (used for height)
                    add x3, x29, khafre_s                            // Address of pyramid khafre in stack
                    bl newPyramid                                       // Call to subroutine
                    mov w0, 15                                      // w0 = 15 (used for width)
                    mov w1, 15                                      // w1 = 15 (used for length)
                    mov w2, 18                                      // w2 = 18 (used for height)
                    add x3, x29, cheops_s                            // Address of pyramid cheops in stack
                    bl newPyramid                                       // Call to subroutine
                    ...
                    add x3, x29, khafre_s                            // Address of pyramid khafre in stack
                    add x4, x29, cheops_s                            // Address of pyramid cheops in stack
                    ...
                    add x3, x29, cheops_s                            // Address of pyramid cheops in stack
                    mov w0, 9                                       // w0 = 9 (used for factor)
                    bl expand                                           // Cal to subroutine
                    add x3, x29, cheops_s                            // Address of pyramid cheops in stack
                    mov w0, 27                                      // w0 = 27 (used for delta of x coord)
                    mov w1, -10                                     // w1 = -10 (used for delta of y coord)
                    bl relocate                                         // Call to subroutine
                    add x3, x29, khafre_s                            // Address of pyramid cheops in stack
                    mov w0, -23                                     // w0 = -23 (used for delta of x coord)
                    mov w1, 17                                      // w1 = 17 (used for delta of y coord)
                    bl relocate                                         // Call to subroutine
ifTrue:                                                                 // Branch to if pyramids are equal sizes, skipping relocation and expand subroutines
                    ...
                    ldp x29, x30, [sp], dealloc                           // Loads the pairs of registers and deallocates the specified amount from the stack
                    ret                                                 // Returns to calling code (in OS)
